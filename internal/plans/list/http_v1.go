...
// Handle handles the request to list subscription plans.
// @Summary List plans
...
// @Success 200 {object} PlanResponse "List of plans"
...
func (h *Handler) Handle(c echo.Context) error {
	...
	if userClaims.Role != "SUPERADMIN" && userClaims.Role != "ADMIN" {
		return response.Error(c, http.StatusForbidden, "ROLE_NOT_ALLOWED", "Only SUPERADMIN or ADMIN can list plans", nil)
	}
