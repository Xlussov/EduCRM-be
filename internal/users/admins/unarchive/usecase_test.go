	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func(repo *mocks.UserRepository) {
				...
				repo.On("UpdateUserStatus", mock.Anything, adminID, true).Return(nil).Once()
			},
			expectedError: nil,
			expectedMsg:   "success",
		},
		...
		},
		{
			...
			caller: domain.Caller{Role: domain.RoleSuperadmin},
			mockSetup: func(repo *mocks.UserRepository) {
				repo.On("GetByID", mock.Anything, adminID).Return(&domain.User{ID: adminID, Role: domain.RoleTeacher, IsActive: false}, nil).Once()
			},
		...
		},
		{
			...
			mockSetup: func(repo *mocks.UserRepository) {
				repo.On("GetByID", mock.Anything, adminID).Return(&domain.User{ID: adminID, Role: domain.RoleAdmin, IsActive: false}, nil).Once()
				repo.On("UpdateUserStatus", mock.Anything, adminID, true).Return(errDB).Once()
			},
			expectedError: errDB,
			expectedMsg:   "",
		},
