...
func TestUseCase_Execute(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(mockUR *mocks.UserRepository, mockGR *mocks.GroupRepository, mockSR *mocks.StudentRepository) {
				...
				mockSR.On("GetByID", mock.Anything, studentID).Return(&domain.Student{ID: studentID}, nil).Once()
				mockGR.On("RemoveStudent", mock.Anything, groupID, studentID, mock.AnythingOfType("time.Time")).Return(nil).Once()
			},
			expectedErr: nil,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			uc := NewUseCase(mockGR, mockUR, mockSR)
			res, err := uc.Execute(context.Background(), userID, tt.role, groupID, studentID)
