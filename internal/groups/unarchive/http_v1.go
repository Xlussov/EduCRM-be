...
// @Router /api/v1/groups/{id}/unarchive [patch]
func (h *Handler) Handle(c echo.Context) error {
	...
	if userClaims.Role != "SUPERADMIN" && userClaims.Role != "ADMIN" {
		return response.Error(c, http.StatusForbidden, "ROLE_NOT_ALLOWED", "Only SUPERADMIN or ADMIN can unarchive groups", nil)
	}
	...
	if err != nil {
		...
		case errors.Is(err, domain.ErrAlreadyActive):
			return response.Error(c, http.StatusBadRequest, "ALREADY_ACTIVE", err.Error(), nil)
