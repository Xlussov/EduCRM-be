...
func (h *Handler) Handle(c echo.Context) error {
	...
	if userClaims.Role != "SUPERADMIN" && userClaims.Role != "ADMIN" {
		return response.Error(c, http.StatusForbidden, "ROLE_NOT_ALLOWED", "Only SUPERADMIN or ADMIN can list teachers", nil)
	}

	branchIDs := make([]uuid.UUID, 0, len(userClaims.BranchIDs))
	...
	res, err := h.usecase.Execute(c.Request().Context(), userClaims.Role, branchIDs, Request{})
	if err != nil {
