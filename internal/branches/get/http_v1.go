...
// @Success 200 {object} Response "Branch details"
...
func (h *Handler) Handle(c echo.Context) error {
	...
	if userClaims.Role != "SUPERADMIN" && userClaims.Role != "ADMIN" {
		return response.Error(c, http.StatusForbidden, "ROLE_NOT_ALLOWED", "Only SUPERADMIN or ADMIN can get branches", nil)
	}
	...
	if err != nil {
		if strings.Contains(err.Error(), "no rows in result set") {
			return response.Error(c, http.StatusNotFound, "NOT_FOUND", "Branch not found", nil)
		}
