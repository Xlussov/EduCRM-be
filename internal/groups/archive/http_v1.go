...
// @Router /api/v1/groups/{id}/archive [patch]
func (h *Handler) Handle(c echo.Context) error {
	...
	if userClaims.Role != "SUPERADMIN" && userClaims.Role != "ADMIN" {
		return response.Error(c, http.StatusForbidden, "ROLE_NOT_ALLOWED", "Only SUPERADMIN or ADMIN can archive groups", nil)
	}
	...
	if err != nil {
		...
		case errors.Is(err, domain.ErrAlreadyArchived):
			return response.Error(c, http.StatusBadRequest, "ALREADY_ARCHIVED", err.Error(), nil)
