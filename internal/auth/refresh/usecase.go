...
func NewUseCase(ur domain.UserRepository, ar domain.AuthRepository, jwtSecret string, accessTTL, refreshTTL time.Duration) *UseCase {
	return &UseCase{
		userRepo:   ur,
		authRepo:   ar,
		jwtSecret:  jwtSecret,
...
func (uc *UseCase) Execute(ctx context.Context, req Request) (Response, error) {
	...
	err = uc.authRepo.SaveRefreshToken(ctx, refreshTokenID, user.ID, newHashStr, time.Now().Add(uc.refreshTTL))
	if err != nil {
		return Response{}, err
	}
